...
deferred in SP8 pending the reference project's grader spec, which is now accessible)
...
The reference project solved this in three parts, all now readable: a `GRADE` trace line the
...
system; the project-specific parts are named and left behind.

## Decision 1 — the trace is the instrument; no new pipeline

The reference project's §7 explicitly rejected a dedicated measurement pipeline as
...
Keys are English (SP8 canon), generalized from the reference project's `issue=`/`ak=`:
...
The reference project's #318 showed a GRADE line with an empty `checkpoint=` silently falling
...
never mutates, never runs in CI. That split is load-bearing: the reference project's design
...
The cockpit ports the reference project's bias-class framing wholesale, because it is the part
...
- **convergence** + **suite** — the grader-trust instruments: kickback
  ...
  not evidence (reference project #318 vacuity guard).
...
- **friction** — deliberately a stub: the bypass-event log is the only piece
  ...
  prove friction matters (reference-project iteration-2 discipline).

## Decision 4 — one parser owner, two scripts, one module

The reference project splits `grade_metrics.py` / `process_kpis.py` for phase-history reasons
...
## Not ported (project-specific, named honestly)
...
the reference project's multi-host issue workflow). The thresholds (20/5, 0 false-pass, 90%,
